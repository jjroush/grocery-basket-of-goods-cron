...
const goodsWebsitesPromises = basketOfGoods.map(async (good) => {
	try {
		const website = await fetch(
			`https://www.target.com/p/${good.nationalGroceryId}`,
		...
		price = window.__TGT_DATA__.__PRELOADED_QUERIES__.queries
			.filter((query) => query.at(0).at(0) === "@web/domain-product/get-pdp-v1")
			.at(0)
			.at(1).data.product.price.current_retail;

		if (!price) {
			price = window.__TGT_DATA__.__PRELOADED_QUERIES__.queries
				.filter(
					(query) => query.at(0).at(0) === "@web/domain-product/get-pdp-v1",
				)
				.at(0)
				.at(1).data.product.price.current_retail_min;
		}
		...
		console.log("Issue Fetching:", good);
		throw new Error(e);
	}
});
